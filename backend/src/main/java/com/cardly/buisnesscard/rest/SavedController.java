...
public class SavedController {
    ...
    public String scanQrCode(@PathVariable String qrHash, Principal principal) {
        ...
        if (exists) {
            return "redirect:/home?error=already_saved";
        }
        ...
        return "redirect:/home?tab=saved";
    }

    @PostMapping("/add-by-hash")
    public String addSavedCardByHash(@RequestParam String qrHash, Principal principal, RedirectAttributes redirectAttributes) {
        String username = principal.getName();
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new IllegalArgumentException("User not found"));
    ...
    public String deleteSavedCard(@PathVariable Long savedId, Principal principal) {
        String username = principal.getName();
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new IllegalArgumentException("User not found"));
